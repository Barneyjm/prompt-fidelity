...
- **Broad questions compose.** A question that fans out into sub-questions across disparate datasets (often via sub-agents) rolls up with `--merge`: each sub-report keeps its own within-dataset joint correction, verified/inferred bits sum across sub-questions (a joint count across different datasets is not measurable — the composite report says so), and the decomposition and synthesis choices — "these metrics capture what the user asked", "complaint volume proxies incidence" — enter as explicit, almost-always-inferred *bridging constraints* via `--bridge`. Injected filters from every sub-question surface in the composite. The composite score is typically lower than any sub-score: a broad question doesn't become verifiable just because its pieces are.
